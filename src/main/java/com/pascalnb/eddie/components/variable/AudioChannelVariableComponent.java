...
public class AudioChannelVariableComponent extends VariableComponent<AudioChannel> implements SettingsMenuItem {

    private final String title;

    public AudioChannelVariableComponent(ComponentConfig config,  String name, String title) {
        super(config,
            name,
    ...
    public static EddieComponentFactory<AudioChannelVariableComponent> factory(String name, String title) {
        return (config) -> new AudioChannelVariableComponent(config, name, title);
    }
            ...
            public @Nullable SettingsMenu apply(EntitySelectInteractionEvent event, InteractionHook hook) {
                try {
                    if (event.getValues().isEmpty()) {
                        ...
                        AudioChannel channel = event.getMentions().getChannels(AudioChannel.class).getFirst();
                        setValue(channel);
                    }
            ...
            public EntitySelectMenu getEntity() {
                return hasValue()
                    ? EntitySelectMenu.create(getId(), EntitySelectMenu.SelectTarget.CHANNEL)
                      .setChannelTypes(ChannelType.STAGE, ChannelType.VOICE)
                      .setRequired(false)
