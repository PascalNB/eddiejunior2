...
public final class URLUtil {
    ...
    private static final String DOMAIN_REGEX = "^[a-zA-Z\\-\\d]+\\.[a-z]{2,}$";
    ...
    @NotNull
    public static Message messageFromURL(@NotNull String url, @NotNull Guild guild) throws CommandException {
        Matcher matcher = Message.JUMP_URL_PATTERN.matcher(url);
        String[] jump;
        if (matcher.find()) {
            jump = new String[]{matcher.group(1), matcher.group(2), matcher.group(3)};
        } else {
            throw new CommandException("Please use a proper jump url for the message");
        }
        if (!guild.getId().equals(jump[0])) {
            throw new CommandException("Please reference a message from this server");
        }
        MessageChannel channel = guild.getChannelById(GuildMessageChannel.class, jump[1]);
        if (channel == null) {
            throw new CommandException("Channel of referenced message not found");
        }
        Message message;
        try {
            message = channel.retrieveMessageById(jump[2]).complete();
            if (message == null) {
                throw new CommandException("Referenced message not found");
            }
        } catch (InsufficientPermissionException e) {
            throw new CommandException("Requires permission %s".formatted(Permission.MESSAGE_HISTORY.getName()));
        }
        return message;
    }
}
